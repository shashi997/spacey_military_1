...
import MissionPreviewDemo from '../components/interactive/MissionPreviewDemo';
...
const LessonPage = () => {
  ...
  useEffect(() => {
    ...
    } else if (lessonId && !currentUser?.uid) {
      // Only load from scratch if no user is logged in
      loadLesson();
    }
  ...
  const saveLessonProgress = async (
    ...
    if (
      ...
      console.warn(
        "[saveLessonProgress] Not saving progress: blockId is null or undefined",
        { userId: currentUser?.uid, lessonId, blockId, tags }
      );
    ...
    try {
      ...
      if (completed) {
        ...
        console.log(
          `📚 Saving lesson completion for ${lessonId}:`,
          progressData
        );
      }
  ...
  const { currentBlock, currentBlockIndex } = useMemo(() => {
    if (!lesson || !currentBlockId)
      return { currentBlock: null, currentBlockIndex: -1 };
    const blockIndex = lesson.blocks.findIndex(
      (b) => b.block_id === currentBlockId
    );
    return {
      currentBlock: lesson.blocks[blockIndex],
  ...
  const handleChoice = useCallback(
    (choice) => {
      ...
      const runAnalysis = async () => {
        try {
          const optimisticTags = tag
            ? [...new Set([...userTags, tag])]
            : [...userTags];
...
Format your response to include both immediate feedback and any trait analysis.`;
          ...
          if (tag) {
            setUserTags((prevTags) => {
              ...
              const updatedPersistentTags = [
                ...new Set([...persistentUserTags, ...updatedTags]),
              ];
  ...
  const getDynamicText = useCallback(
    (dynamicContent) => {
      ...
      const defaultItem = dynamicContent.find(
        (item) => !item.condition_tags || item.condition_tags.length === 0
      );

      dynamicContent.forEach((item) => {
        if (!item.condition_tags || item.condition_tags.length === 0) return;
        const score = item.condition_tags.filter((tag) =>
    ...
    switch (pageState) {
      ...
      default:
        ...
        if (
          ...
          (augmentedBlock.type === "narration" ||
            augmentedBlock.type === "reflection" ||
            augmentedBlock.type === "quiz") &&
          currentBlockIndex < lesson.blocks.length - 1
        ) {
          augmentedBlock.next_block =
            lesson.blocks[currentBlockIndex + 1].block_id;
        }
        ...
        switch (augmentedBlock.type) {
          case "narration":
            ...
            if (
              augmentedBlock.block_id === "Debrief" ||
              augmentedBlock.block_id === "Mission Complete" ||
              !augmentedBlock.next_block
            ) {
              console.log(
                "🎯 Triggering lesson completion for block:",
                augmentedBlock.block_id
              );
          ...
          case "interactive_demo":
            switch (augmentedBlock.demo_component) {
              ...
              default:
                return (
                  <p>
                    Unknown interactive demo: {augmentedBlock.demo_component}
                  </p>
                );
            }
          default:
    ...
    if (error) {
      return (
        <div className="flex flex-col items-center justify-center gap-4 text-red-400/80 h-48 bg-red-900/20 p-8 rounded-lg">
          ...
          <Link to="/dashboard" className="mt-4 text-cyan-300 hover:underline">
            Return to Dashboard
          </Link>
        </div>
      );
      ...
      <>
        <div className="mb-8">
          <div className="flex justify-between items-center mb-2 font-mono text-sm text-cyan-300">
            ...
            <span>
              Block {currentBlockIndex + 1} of {lesson.total_blocks}
            </span>
          </div>
          <div className="w-full bg-black/30 rounded-full h-2.5">
            <div
              ...
              style={{
                width: `${
                  ((currentBlockIndex + 1) / lesson.total_blocks) * 100
                }%`,
              }}
            ></div>
          </div>
...
Respond as if you're right there with the user during their mission.`;
        ...
        newHistory = [
          ...newHistory,
          { sender: "ai", content: fallbackResponse },
        ];
        setChatHistory(newHistory);
        saveLessonProgress(
          currentBlockId,
    ...
    <div className="min-h-screen bg-black text-white overflow-hidden relative">
      ...
      <Navbar
        rightControls={
          !isLoading && !error ? (
            <div className="flex items-center gap-2">
              {lessonId && <LessonProgressIndicator lessonId={lessonId} />}{" "}
              {/* Render here */}
              {lessonNavControls}
            </div>
          ) : null
      ...
      <main className="relative z-10 min-h-screen pt-20 pb-24 px-4 md:px-8">
        ...
        <div className="w-full max-w-screen-2xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-10 gap-6 lg:gap-8 min-h-[500px]">
            ...
            <div className="lg:col-span-6">{renderLessonContent()}</div>
...
// Usage example for dashboard refresh after mission completion:
// import { useDashboardRefresh } from '../pages/UserDashboardPage';
// const { refreshDashboard } = useDashboardRefresh(currentUser?.uid);
// Call refreshDashboard() after mission completion in LessonPage.jsx
