...
const SignupPage = () => {
  ...
  const handleSignup = async (e) => {
    e.preventDefault()
    // In a real app, you'd handle signup logic here, e.g., API calls.
    // For this example, we'll just simulate a signup process.
    setLoading(true)
    setError(null)
    console.log('Clicked Create Account:', { name, email, password }) // For now
      ...
      <main className="relative z-20 flex items-center justify-center h-full px-4">
        <div className="w-full max-w-md p-8 space-y-6 bg-black/40 backdrop-blur-md rounded-xl border border-white/10 animate-[fadeIn_1s_ease-in-out]">
          <div className="text-center">
            <h1 className="text-3xl font-oswald font-bold text-gray-100">Create an Account</h1>
          ...
          <p className="text-sm text-center text-gray-400">
            ...
            <Link to="/login" className="font-medium text-yellow-400 hover:underline">Log In</Link>
          </p>
        </div>
      </main>
    </div>
